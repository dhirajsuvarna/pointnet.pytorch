...
def generate_image(iFilePath):
    """ Demo for draw_point_cloud function """
    #filePath = r"F:\projects\ai\pointnet\dataset\bone_models\pcd\Femur (11).pcd"
    cloud = o3d.io.read_point_cloud(iFilePath)
    points = np.asarray(cloud.points)
    im_array = point_cloud_three_views(points)
    img = Image.fromarray(np.uint8(im_array*255.0))
    outPath = iFilePath.replace('.pcd', '.png')
    img.save(outPath)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inputPath", required=True, help="File or Folder containing the PCD files")
    ...
    if os.path.isdir(inputPath):
        #itereate directories and created image
        for root, subdirs, files in os.walk(inputPath):
            for fileName in files:
                if fileName.endswith('.pcd'):
                    filePath = os.path.join(root, fileName)
                    print(f"Generating Image for: {filePath}")
                    generate_image(filePath)
    else:
        if inputPath.endswith('.pcd'):
            generate_image(inputPath)
        else:
            print(f"{inputPath}: Invalid file")