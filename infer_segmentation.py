...
def infer_segments(model, cadFilePath):
    ...
    outPcdFile = os.path.join(output_folder, os.path.splitext(os.path.basename(cadFilePath))[0] + "_out.pcd")
    o3d.io.write_point_cloud(outPcdFile, pcd, write_ascii=True)


#input check folder or file
if not os.path.isdir(cad_model_folder):
    print(f"{cad_model_folder} : not a directory")
    if not cad_model_folder.endswith('.pcd'):
        print(f"{cad_model_folder}: not a valid pcd file")
        sys.exit()
    
    infer_segments(model, cad_model_folder)
else:
    for root, subdirs, files in os.walk(cad_model_folder):
        for fileName in files:
            ...
            infer_segments(model, cadFilePath)
